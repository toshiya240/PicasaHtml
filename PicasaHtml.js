...
function loadConfig() {
    var confUserID = storage.get("conf_picasahtml_user");
...
function saveConfig() {
    ...
    storage.set("conf_picasahtml_user", $("#conf-userID").val());
...
function clearConfig() {
    //storage.clear();
    storage.set("conf_picasahtml_user", "");
    storage.set("conf_picasahtml_fmt_index", "0");
    storage.set("conf_picasahtml_fmt_custom", "");
    loadConfig();
...
$(document).on("pagebeforecreate", "#main", function() {
    // in-app だと $() が呼び出されないようなのでここで実行
    var $placeholderSelect = $("#placeholder");
    for (var label in placeholder) {
        $placeholderSelect.append($("<a data-role='button'>").text(label).attr(
                'href', 'javascript:insertPlaceholder("'+label+'");'));
    }
